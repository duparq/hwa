...
/**
 * @file
 * @brief HD44780 LCD controller
 */

/**
 * @ingroup ext
 * @defgroup hd44780 HD44780 LCD controller
 *
 ...
 * @endcode
 *
 * @section hd44780ctr Constructor
 *
 * <br><br>HW_HD44780(...) declares a HD44780 object. You must provide:
 *  * the number of display lines;
 *  * the number of display columns;
 *  * the I/O the 'e' pin is connected to;
 *  * the I/O the 'rs' pin is connected to;
 *  * the I/O the 'rw' pin is connected to;
 *  * the I/O the 'data' pins are connected to.
 *
 * @code
 ...
 *
 * The implementation of this device focuses on code size rather than on speed,
 * so it relies on extern C functions rather than on inlined code. You must use
 * HW_IMPLEMENT() to have these functions defined.
 *
 * <br><br>HW_DECLARE( LCD ) declares the functions that implement the device. You
 ...
 * @endcode
 *
 * <br><br>HW_IMPLEMENT( LCD ) defines the functions that implement the
 * device. This must appear in one of your source files:
 *
 * @code
 * HW_IMPLEMENT( LCD );
 * @endcode
...
/**
 * @ingroup hd44780
 * @brief Constructor for HD44780 LCD controtroller
 * @hideinitializer
 */
...
#define _HW_HD44780_70(x,...)			HW_E(HW_EM_G(x))
...
#define _HW_HD44780_E(k,...)						\
  ,HW_HD44780(...),(HW_EM_SY("HW_HD44780(lines,..., cols,..., e,..., rs,..., rw,..., data,...)"))
...
#define _hw_dchd44780(o,ec,dw,l,c,e,rs,rw,d,...)	HW_BV(_hw_dchd44780,declare_,HW_A0(__VA_ARGS__),o,ec,dw)
#define _hw_dchd447800(v,...)		HW_E(HW_EM_OVL(v,(weak)))
#define _hw_dchd447801(weak,o,ec,dw)					\
  void weak _hw_##o##ec##_##dw##__pulse ( );				\
  void weak _hw_##o##ec##_##dw##__write ( uint8_t, uint8_t );		\
  uint8_t weak _hw_##o##ec##_##dw##__read ( uint8_t );			\
  uint8_t weak _hw_##o##__wait ( );					\
  void weak _hw_##o##__command ( uint8_t );				\
  void weak _hw_##o##__data ( uint8_t );				\
  void weak _hw_##o##_clear_cgram ( );					\
  void weak _hw_##o##_cls ( );						\
  void weak _hw_##o##ec##_##dw##_init ( );				\
  void weak _hw_##o##_gotoxy ( uint8_t, uint8_t );			\
  void weak _hw_##o##_newline( uint8_t );				\
  void weak _hw_##o##_home ( );						\
  void weak _hw_##o##_putchar( unsigned char ) /* require a ; */


/**
 * @ingroup hd44780
 * @brief Defines the functions that implement the class.
 * @hideinitializer
 */
...
/**
 * @addtogroup hd44780
 * @section hd44780rel Relatives
 *
 * * `(LCD,putchar)`: the name of the putchar function associated to the LCD.
 *
 * @code
 * HW(LCD,putchar)('x');	//  Write 'x' on the LCD using its putchar function
 * @endcode
 *
 * Assuming that `myprintf` takes the address of a `putchar` function as first
 * argument:
 *
 * @code
...
/**
 * @addtogroup hd44780
 * @section hd44780act Actions
 *
 * <br><br>hw( configure, LCD, ... ) configures the LCD:
 *
 * @code
 * hw(   configure, LCD,
 *
...
#define _hw_cfhd44780_ini1(k,v,...)	HW_BV(_hw_cfhd44780_ini2,state_,v,)(__VA_ARGS__) // PUSH
#define _hw_cfhd44780_ini20(v,...)	HW_E(HW_EM_ST(v)) HW_EAT // POP
#define _hw_cfhd44780_ini21(v,...)	init=v; _hw_cfhd44780_dis // POP
...
#define _hw_cfhd44780_dis11(k,v,...)	HW_BV(_hw_cfhd44780_dis2,state_,v,)(__VA_ARGS__)
...
#define _hw_cfhd44780_cur10(k,...)	HW_E(HW_EM_AN(k,cursor))
#define _hw_cfhd44780_cur11(k,v,...)	HW_BV(_hw_cfhd44780_cur2,state_,v,)(__VA_ARGS__)
...
#define _hw_cfhd44780_blk10(k,...)	HW_E(HW_EM_AN(k,blink))
#define _hw_cfhd44780_blk11(k,v,...)	HW_BV(_hw_cfhd44780_blk2,state_,v,)(__VA_ARGS__)
...
#define _hw_cfhd44780_shf11(k,v,...)	HW_BV(_hw_cfhd44780_shf3,hd44780_shift_,v,)(__VA_ARGS__)
...
#define _hw_cfhd44780_drn10(k,...)	HW_E(HW_EM_AN(k,direction))
#define _hw_cfhd44780_drn11(k,v,...)	HW_BV(_hw_cfhd44780_drn2,hd44780_direction_,v,)(__VA_ARGS__)
 ...
 * @addtogroup hd44780
 *
 * <br><br>hw( cls, LCD ) clears the screen:
 *
 * @code
 * hw( cls, LCD );
 * @endcode
 ...
 * @addtogroup hd44780
 *
 * <br><br>hw( gotoxy, LCD, x, y ) moves the cursor to column x, line y:
 *
 * @code
 * hw( gotoxy, LCD, x, y );
 * @endcode
 ...
 * @addtogroup hd44780
 *
 * <br><br>hw( newline, LCD ) moves the cursor to the start of the next line:
 *
 * @code
 * hw( newline, LCD, pos );
 * @endcode
 ...
 * @addtogroup hd44780
 *
 * <br><br>hw( home, LCD ) moves the cursor to the home position:
 *
 * @code
 * hw( home, LCD );
 * @endcode
 ...
 * @addtogroup hd44780
 *
 * <br><br>hw( putchar, LCD, c ) writes a character at the current position:
 *
 * @code
 * hw( putchar, LCD, c );
 * @endcode
