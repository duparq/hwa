#ifndef HWA_ATMEGA88_GPIO_H
#define HWA_ATMEGA88_GPIO_H
...
#define HWA_PORTB_BASE			0x23
#define HWA_PORTC_BASE			0x26
#define HWA_PORTD_BASE			0x29

#define HWA_GPIO_HWA_PIN		volatile, u8, 0x00, 0x00, 0xFF
#define HWA_GPIO_HWA_DDR		volatile, u8, 0x01, 0x00, 0xFF
#define HWA_GPIO_HWA_PORT		volatile, u8, 0x02, 0x00, 0xFF
...
#define hwa_commit_gpio()			\
  do {						\
    ...
    hwa_commit_gpio_port(HWA_PORTC);		\
    hwa_commit_gpio_port(HWA_PORTD);		\
  } while(0)
...
/*   ((HW_HR(HWA_GPIO, pname, HWA_IDR) & ((mask)<<(shift))) != 0) */
...
#define hw_gpio_write_pins(pname, mask, value)				\
  do {									\
    if ( mask == 0xFF )							\
      HW_HR(HWA_GPIO, pname, HWA_PORT) = value ;			\
    else if ( mask != 0 )						\
      HW_HR(HWA_GPIO, pname, HWA_PORT) =				\
...
#define hwa_gpio_config_pin_(pptr, bit, mode)				\
  do {									\
    ...
    if ( mode == 0b100 ) /* Analog inputs on PC0..PC5 */ {		\
      if ( pptr->hwaddr != HWA_PORTC_BASE ||				\
	   bit < 0 || bit > 5 )						\
	HWA_ERROR("Can not configure pin for analog input.");		\
      /* HWA_SETVB(HWA_ADC, HWA_ADC0, HWA_ADC_DIDR0, 0b1, bit, 1); */	\
    }									\
  } while(0)
