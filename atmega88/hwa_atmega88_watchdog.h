...
#define hwa_watchdog_set_timeout(timeout)			\
  do {								\
    HWA_SETVB(u8, WATCHDOG, CR, 0b1,   4, 1); /* WDCE=1 */	\
    HWA_SETVB(u8, WATCHDOG, CR, 0b1,   5, HWA_WDTO_##timeout >> 3);	\
    HWA_SETVB(u8, WATCHDOG, CR, 0b111, 0, HWA_WDTO_##timeout &  7);	\
  } while(0)
...
#define hwa_watchdog_set_action(action)				\
  do {								\
    HWA_SETVB(u8, WATCHDOG, CR, 0b1, 4, 1); /* WDCE=1 */		\
    HWA_SETVB(u8, WATCHDOG, CR, 0b1, 3, HWA_WDACTION_##action >> 1);	\
    HWA_SETVB(u8, WATCHDOG, CR, 0b1, 6, HWA_WDACTION_##action &  1);	\
  } while(0)
