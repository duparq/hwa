...
#define _hwa_begin_c16a_2(c,n,i,a)		\
  ...
  hwa->n.update = 0;				\
  hwa->n.overflow = 0;				\
  hwa->n.output0_mode = 0;				\
  hwa->n.output1_mode = 0;				\
  hwa->n.icr_input = 0;				\
  hwa->n.icr_edge = 0;				\
  hwa->n.icr_filter = 0;
...
#define hw_c16a_top_input0			, 5
#define hw_c16a_top_output0			, 6
...
HW_INLINE void hwa_solve_c16a ( hwa_t *hwa, hwa_c16a_t *p )
{
  ...
  if ( p->countmode ) {	/* if the mandatory parameter 'countmode' is not 0, the
	...
	wgm = 7 ;
      else if (p->top == HW_A1(hw_c16a_top_output0) ) {
	if (p->update == HW_A1(hw_counter_update_immediately)
      ...
      else /* if (p->top == HW_A1(hw_c16a_top_input0) ) */ {
...
#  warn "TODO: check the validity of this"
   ...
   */
  if ( p->countmode || p->output0_mode || p->output1_mode ) {
